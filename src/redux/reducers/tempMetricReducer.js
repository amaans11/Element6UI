...
export default function strandedAssetReducer(
  state = { ...intialState },
  action,
) {
  switch (action.type) {
    ...
    case types.GET_SECTORAL_TEMP_SCORE_SUCCESS:
      return produce(state, (draft) => {
        draft.sectoralTempScore.data = action.res
        draft.sectoralTempScore.error = ''
      })
    case types.GET_SECTORAL_TEMP_SCORE_FAILED:
      return produce(state, (draft) => {
        draft.sectoralTempScore.data = ''
        draft.sectoralTempScore.error = action.error
      })
