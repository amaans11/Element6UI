...
const PortfolioOptimization = () => {
  ...
  const optimizationData = useSelector(
    (state) => state.optimization.optimizationData,
  )
  ...
  const getIntensityData = (response) => {
    ...
    const intensityPortSc12 = parseFloat(
      response['tilted']['Intensities']['portfolio']['Scope_12'].toFixed(2),
    )
    const intensityPortSc123 = parseFloat(
      response['tilted']['Intensities']['portfolio']['Scope_123'].toFixed(2),
    )
    const intensityPortSc3 = parseFloat(
      response['tilted']['Intensities']['portfolio']['Scope_3'].toFixed(2),
    )

    const intensityTiltedSc12 = parseFloat(
      response['tilted']['Intensities']['tilted']['Scope_12'].toFixed(2),
    )
    const intensityTiltedSc123 = parseFloat(
      response['tilted']['Intensities']['tilted']['Scope_123'].toFixed(2),
    )
    const intensityTiltedSc3 = parseFloat(
      response['tilted']['Intensities']['tilted']['Scope_3'].toFixed(2),
    )
  ...
  const getData = async () => {
    ...
    if (
      ...
      Object.keys(optimizationData['data'].length > 0)
    ) {
      ...
      const portSectorWeights =
        response['tilted']['Intensities']['portfolio']['sector_weight']
      const tiltedSectorWeights =
        response['tilted']['Intensities']['tilted']['sector_weight']
      const portSectorContribs =
        response['tilted']['Intensities']['portfolio']['sectoral_contribution']
      const tiltedSectorContribs =
        response['tilted']['Intensities']['tilted']['sectoral_contribution']
          ...
          <Grid container style={{ marginLeft: 20, marginTop: 10 }}>
            <Grid item xs={5}>
              <Box style={{ fontSize: 14 }}>
                Select Urgentem reweight factor
              </Box>
              <Slider
