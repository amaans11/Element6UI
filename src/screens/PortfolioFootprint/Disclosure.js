...
const Disclosure = () => {
  ...
  const benchDisclosure = useSelector(
    (state) => state.footprint.benchDisclosure,
  )
  ...
  const getColumnChartData = () => {
    ...
    if (portDisclosure['data'] && benchDisclosure['data']) {
      const portData =
        portDisclosure && Object.keys(portDisclosure).length > 0
          ? portDisclosure['data']['Scope3_disc']
          : []
      const benchData =
  ...
  const getStackedChartData = () => {
    ...
    if (portDisclosure['data'] && benchDisclosure['data']) {
      const portData =
        portDisclosure && Object.keys(portDisclosure).length > 0
          ? portDisclosure['data']['Scope12_disc']
          : []
      const benchData =
        benchDisclosure && Object.keys(benchDisclosure).length > 0
      ...
      if (portData && portData.length > 0) {
        portData.map((res, index) => {
          ...
          let benchValue =
            benchData && benchData.length > 0 && benchData[index]['Proportion']
    ...
    <React.Fragment>
      {loading ? (
        ...
        <Box>
          ...
          >
            This chart highlights the number of Scope 3 categories disclosed out
            of a possible 15 for company holdings with the portfolio and
            benchmark as a percentage. This relates to the issuers that Urgentem
